...
import com.chenyao.note.dao.NoteTypeDao;
...
//    2. 调用Dao层，查询当前登录用户下，类型名称是否唯一，返回0或1（1=可用；0=不可用）
        Integer code = BaseDao.checkTypeName(typeName,userId,typeId);
...
//    如果为空，调用Dao层的添加方法，返回主键 （前台页面需要显示添加成功之后的类型ID）
        if (StrUtil.isBlank(typeId)){
            key = BaseDao.addType(typeName,userId);
        }
        else {
//    如果不为空，调用Dao层的修改方法，返回受影响的行数
            key = BaseDao.updateType(typeName,typeId);
        }
